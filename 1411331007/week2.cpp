...
int main(void)
{
    ...
    if ((fopen_s(&cfPtr,"accounts.dat", "rb+"))!=0) {
        ...
        printf("%s", "Enter account number"
            " (1 to 2, 0 to end input): ");
        scanf_s("%d", &client.ID);

        // user enters information, which is copied into file
        while (client.ID != 0) {
            // user enters last name, first name and balance
            printf("%s", "Enter name, math, program: ");
            scanf_s("%14s%9d%d",&client.Name,14,&client.math, &client.program);
            client.average = (client.math + client.program) / 2;
